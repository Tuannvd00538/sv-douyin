...
<div class="container">
    <form method="POST">
        <div class="form-group">
            <label for="url">Vui lòng nhập url video:</label>
            <input type="text" class="form-control" placeholder="https://v.douyin.com/n3w9Mv/" name="url" id="url">
        </div>
        <input type="submit" name="getLinkDownload" class="btn btn-success" value="Tải Video" />
    </form>
    <?php
    ...
    if (isset($_POST['getLinkDownload'])) {
        if ($_POST['url']) {
            $url = $_POST['url'];
